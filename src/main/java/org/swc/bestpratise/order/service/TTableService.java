...
import org.swc.bestpratise.order.repository.TTableDao;
...
public class TTableService {
	@Autowired
	private TTableDao dao;

	public Object saveEntity(TTable entity) throws Exception {
		return dao.saveEntity(entity);
	}
	
	public Object checkSave(TTable entity) throws Exception {
		TTable data = dao.findEntityBy(new String[]{"name","project_id"}, new Object[]{"'"+entity.getName()+"'",entity.getProjectId()});
		if(data == null)
		{
			return dao.saveEntity(entity);
		}else
		{
			...
			dao.updateEntity(data);
			return data.getId();
		}
	}

	@Transactional
	public void update(TTable entity) throws Exception {
		dao.updateEntity(entity);
	}

	public TTable findEntityById(Long id) throws Exception {
	...
	public List<TTable> findEntityList(String[] keyName, String[] operate,
	...
	public List<TTable> myEntityList(String userId) throws Exception {
	...
	public Page getEntityPage(ServletRequest request) throws Exception {
		...
		List<TTable> data = dao
				.findEntityList(names, opt, params, 1000, pageNO);
	...
	public Page getAllEntityPage(HttpServletRequest request) throws Exception {
		...
		int count = dao.getEntityCount(new String[] {}, new String[] {},
				new Object[] {}, null);
		List<TTable> data = dao.findEntityList(new String[] {},
	...
	public List<TTable> getTable(String source,String user,String password)
			throws Exception {
        ...
        rsets =  data.getTables(null, "%", "%", new String[]{"TABLE"});
        List<TTable> tc = new ArrayList<TTable>();
 	   while(rsets.next()){
		   String name = rsets.getObject(3).toString();
//		   tables.add(name);
		   TTable tabl = new TTable();
		   tabl.setCreateTime(new Date());
		   tabl.setName(name);
		   tc.add(tabl);
	   }
 	  rsets.close();
